defmodule OrientDemo.HomeScreen do
  @moduledoc "Landing screen for OrientDemo."
  use Mob.Screen
  ...
  def render(assigns) do
    ...
    <Scroll background={:background}>
      <Column background={:background} padding={:space_lg}>
        ...
        <Spacer size={16} />
        <Text text="OrientDemo" text_size={:xl} text_color={:on_surface} padding={:space_sm} />
        <Text text="BEAM running on device" text_size={:sm} text_color={:primary} padding={4} />
        <Spacer size={40} />
        {nav_button("Orientation",         :open_orientation)}
        <Spacer size={12} />
  ...
  def handle_info({:tap, :open_orientation}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, OrientDemo.OrientationScreen)}
  end

  def handle_info({:tap, :open_text}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, OrientDemo.TextScreen)}
  end

  def handle_info({:tap, :open_list}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, OrientDemo.ListScreen)}
  end

  def handle_info({:tap, :open_dice}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, OrientDemo.DiceScreen)}
  end

  def handle_info({:tap, :open_webview}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, OrientDemo.WebViewScreen)}
  end

  def handle_info({:tap, :open_audio}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, OrientDemo.AudioScreen)}
  end

  def handle_info({:tap, :open_storage}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, OrientDemo.StorageScreen)}
  end
